...
$(document).ready(function () {
    ...
    $("select[name='shequ']").change(function () {
        ...
        } else if (type === 1 || type === 3 || type === 5 || type === 11 || type === 12 || type === 13) { //时空云社区编辑商品
            $("#goods_type").css("display", "none");
        ...
        if (type == 'jiuwu') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.code == 0) {
                        $("input[name='shopimg']").val(data.image);
                        var paramname = data.param2;//输入框标题
                        var inputs = '';
                        $.each(paramname.split('|'), function (i, v) {
                            if (i == 0) {
                                $("input[name='input']").val(v);
                        ...
                        $("#value").attr('min', +data.limit_min);
                        $("#value").attr('max', +data.limit_max);
                        $("input[name='goods_param']").val(data.param);//参数名
                        if ($("input[name='name']").val() == '') $("input[name='name']").val(name);
                        if ($("textarea[name='desc']").val() == '') $("textarea[name='desc']").val(data.desc);
                        if ($("input[name='shopimg']").val() == '') $("input[name='shopimg']").val(data.image_url);

                        $("#GoodsInfo").html('<b>商品名称：</b><a style="color:white" href="http://' + $("select[name='shequ'] option:selected").attr('data-community-domain') + '/home/order/' + goodsid + '" target="_blank" rel="noreferrrer">' + name + '</a><br/>社区商品售价：</b>' + data.price + ' 元<br/><b>最小下单数量：</b>' + data.limit_min + '<br/><b>最大下单数量：</b>' + data.limit_max +'</br><b>商品描述：</b>' + data.desc + ' <br/><b>');
                        $("#GoodsInfo").slideDown();
                        changeNum();
                    } else {
                        layer.alert(data.msg);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.code == 0) {
                        ...
                        $("input[name='goods_param']").val(data.param);//参数名
                        if ($("input[name='name']").val() == '') $("input[name='name']").val(name);
                        if ($("input[name='input']").val() == '') $("input[name='input']").val(account_name);
                        if ($("textarea[name='desc']").val() == '') $("textarea[name='desc']").val(data.desc);


                        $("#GoodsInfo").html('<b>商品名称：</b><a style="color:white" href="http://' + $("select[name='shequ'] option:selected").attr('data-community-domain') + '/home/order/' + goodsid + '" target="_blank" rel="noreferrrer">' + name + '</a><br/><b>社区商品售价：</b>' + data.price + ' 元<br/><b>最小下单数量：</b>' + data.limit_min + '<br/><b>最大下单数量：</b>' + data.limit_max);
                        $("#GoodsInfo").slideDown();
                        changeNum();
                    } else {
                        layer.alert(data.msg);
