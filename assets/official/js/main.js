...
function getcount() {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#order_count').text(data['orders'] ? data['orders'] + '+' : '0'); // 订单总数
            $('#count_orders1').html(data['orders1']);
            $('#today_order_count').text(data['orders2'] ? data['orders2'] + '+' : '0'); // 今日订单
            $('#count_orders_all').html(data.orders);
            $('#count_orders_today').html(data.orders2);
            $('#money_count').text(data['money'] ? data['money'] + '+' : '0'); // 交易总数
            $('#today_money_count').text(data['money1'] ? data['money1'] + '+' : '0'); // 今日交易
...
function getxiaohongshuid() {
    ...
    if (songurl.indexOf('/t.cn/') > 0 || songurl.indexOf('xhsurl.com') > 0 || songurl.indexOf('w.url.cn') > 0 || songurl.indexOf('xhslink.com') > 0) {
        var ii = layer.load(2, {shade: [0.1, '#fff']});
        $.ajax({
            type: "POST",
        ...
        success: function (data) {
            if (data.code == 0) {
                ...
                $.each(data.data, function (i, item) {
                    ...
                    $('#list').append('<tr orderid=' + item.id + '><td>' + item.input + '</td><td>' + item.name + '</td><td>' + item.value + '</td><td class="hidden-xs">' + item.addtime + '</td><td>' + status + '</td><td><a onclick="showOrder(\'' + item.id + '\',\'' + item.skey + '\',\'' + item.order_id + '\',' + item.order_set + ')" title="查看订单详细" class="btn btn-info btn-xs">详细</a></td></tr>');
                    if (item.result != null) {
                        if (item.status == 3) {
                ...
                $("#result2").slideDown();
                if ($_GET['buyok']) {
                    showOrder(data.data[0].id, data.data[0].skey, data.data[0].order_id , data.data[0].order_set)
                }
            } else {
                layer.alert(data.msg);
            }
...
function showOrder(id, skey,order_id,order_set) {
    var ii = layer.load(2, {shade: [0.1, '#fff']});
    var status = [
        ...
        '<span class="label label-danger" style="background-color: red;">已退款</span>',
        '<span class="label label-primary">已提交</span>'
    ];
    $.ajax({
        type: "POST",
        url: "ajax.php?act=order",
        data: {id: id, skey: skey,order_id:order_id,order_set:order_set},
        dataType: 'json',
        success: function (data) {
            layer.close(ii);
            ...
            if (data.code === 0) {
                ...
                if (order_set != 1)
                    ...
                    item += '<tr><td colspan="6" style="text-align:center"><b>订单基本信息</b></td></tr><tr><td class="info">订单编号</td><td colspan="5">' + order_id + '</td></tr><tr><td class="info">商品名称</td><td colspan="5">' + data.name + '</td></tr><tr><td class="info">订单金额</td><td colspan="5">' + data.money + '元</td></tr><tr><td class="info">购买时间</td><td colspan="5">' + data.date + '</td></tr><tr><td class="info">下单信息</td><td colspan="5">' + data.inputs + '</td><tr><td class="info">订单状态</td><td colspan="5">' + status[data.status] + '</td></tr>';
                ...
                if (
                    data["list"]
                    && (parseInt(data["list"]["add_time"]) === 0 || data["list"]["add_time"] === null || data["list"]["add_time"] === undefined)
                ) {
                    // item += '<tr><td colspan="6" style="text-align:center"><b>暂无订单实时状态</b></td><tr><td class="warning">下单数量</td><td>暂无</td><td class="warning">下单时间</td><td colspan="3">暂无</td></tr><tr><td class="warning">初始数量</td><td>暂无</td><td class="warning">当前数量</td><td>暂无</td><td class="warning">订单状态</td><td><font color=blue>暂无</font></td></tr>';
                } else if (data.list && data.list.order_state) {
                    ...
                    if (data['pay_alipay'] > 0) {
                        if (isPayTools && data['pay_tools']['alipay']['btn_type'] === '2' && data['pay_tools']['alipay']['btn_href']) {
                            paymsg += `<a class="btn btn-default btn-block" href="${data['pay_tools']['alipay']['btn_href']}" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/alipay.ico" alt="alipay_logo" class="logo">
                            ...
                            paymsg += `<button class="btn btn-default btn-block" onclick="dopay('alipay', '${data['trade_no']}')" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/alipay.ico" alt="alipay_logo" class="logo"> 支付宝
                                    </button>`;
                        }
                    }
                    if (data.pay_qqpay > 0) {
                        if (isPayTools && data['pay_tools']['qqpay']['btn_type'] === '2' && data['pay_tools']['qqpay']['btn_href']) {
                            paymsg += `<a class="btn btn-default btn-block" href="${data['pay_tools']['qqpay']['btn_href']}" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/qqpay.ico" alt="qqpay_logo" class="logo">
                            ...
                            paymsg += `<button class="btn btn-default btn-block" onclick="dopay('qqpay', '${data['trade_no']}')" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/qqpay.ico" alt="qqpay_logo" class="logo"> QQ钱包
                                    </button>`;
                        }
                    }
                    if (data.pay_wxpay > 0) {
                        if (isPayTools && data['pay_tools']['wxpay']['btn_type'] === '2' && data['pay_tools']['wxpay']['btn_href']) {
                            paymsg += `<a class="btn btn-default btn-block" href="${data['pay_tools']['wxpay']['btn_href']}" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/wechat.ico" alt="wechat_logo" class="logo">
                            ...
                            paymsg += `<button class="btn btn-default btn-block" onclick="dopay('wxpay', '${data['trade_no']}')" style="margin-top:10px;">
                                        <img style="width: 16px;height: 16px;" src="/assets/icon/wechat.ico" alt="wechat_logo" class="logo"> 微信支付
                                    </button>`;
                        }
                    }
                    if (data.pay_rmb > 0) {
